...
| `W4-13` Docker + e2e | [`tasks/w4-13-docker.md`](tasks/w4-13-docker.md) | 8 lượt `/chat` qua container ($0,0105) | tiêm 4 vào hạ tầng: 4/4 đỏ | [`probes/w4-13-e2e-latency.json`](probes/w4-13-e2e-latency.json) | ⭐⭐ p95 end-to-end **5.312 ms vs ngân sách 3.500 — KHÔNG ĐẠT**, phần vượt là bộ sinh chứ không phải truy hồi; GPU đi qua được container nên `runtime_drift: null`; ba lỗi chỉ lộ khi chạy container |
| `W5-03` LLM judge | [`tasks/w5-03-judge.md`](tasks/w5-03-judge.md) | 108 lời gọi 3 nhánh + 3 lần chạy cache ($0,041) | tiêm 14: **13 đỏ**, J2 sống sót vì là mã chết → gỡ | [`probes/w5-03-judge-arms.json`](probes/w5-03-judge-arms.json) · [`probes/w5-03-cache-two-runs.json`](probes/w5-03-cache-two-runs.json) | ⭐⭐ `deepseek-reasoner` **được phục vụ bởi** `deepseek-v4-flash` — ghim nó là ghim một con trỏ; ⭐⭐ suy luận bật không mua thêm phán quyết đúng nào (12/12 cả ba nhánh) mà 5/5 lời gọi hỏng đều cụt đúng ở `max_tokens`; ⭐⭐ cache là thứ **duy nhất** làm eval tái lập được (lần 2: $0 / 0,01 s) |
| `W5-01`+`W5-02` Eval tầng sinh | [`tasks/w5-01-generation-eval.md`](tasks/w5-01-generation-eval.md) | 242 câu qua container ($0,259) + 947 phán quyết judge ($0,508) | tiêm 12: **11 đỏ**, G1 sống sót vì là mã chết → gỡ | [`runs/w5-answers-v1.jsonl`](runs/w5-answers-v1.jsonl) · [`runs/w5-answers-v1-generation.json`](runs/w5-answers-v1-generation.json) | ⭐⭐ harness tìm ra **hai lỗi production** trước khi in con số nào (image trôi khỏi lock; hai request đồng thời làm hỏng tokenizer); ⭐⭐ rubric v1→v2 đổi `uncited_grounding` **0,427 → 0,856** trên cùng dữ liệu; faithfulness **0,9877** ✅, citation accuracy cấp quote **0,8308** ❌ |
| `W5-04` Hiệu chỉnh judge | [`tasks/judge-calibration.md`](tasks/judge-calibration.md) | 50 nhãn tay + 50 lời gọi GLM ($0,0114) + 50 lời gọi suy luận-bật ($0,0408); lấy mẫu và phát lại $0 | tiêm 17: **17 đỏ**, không phép nào sống sót | [`runs/w5-04-calibration.json`](runs/w5-04-calibration.json) · [`runs/w5-04-calibration-reasoning-arm.json`](runs/w5-04-calibration-reasoning-arm.json) · [`runs/w5-04-faith-cross-glm.jsonl`](runs/w5-04-faith-cross-glm.jsonl) | ⭐⭐ **judge hỏng cho điểm 1,0000 chứ không cho điểm thấp** — bật suy luận mất 32/50 phán quyết, và ca mất có ngữ cảnh dài gấp đôi nên **cả 3 mệnh đề thất bại thật đều bị loại**; ⭐⭐ đổi **mỗi** model judge làm faithfulness dịch **7,5 điểm** (0,9246 GLM ↔ 1,0000 suy luận-bật); ⭐⭐ file "mù" **rò rỉ qua thứ tự dòng**, phát hiện ở mẫu 18/50; κ judge–người **0,737** ✅, faithfulness theo nhãn tay **0,9905** [0,9800–1,0000] bao con số của `W5-01` |
| `W5-05` Gate phát hành | [`tasks/w5-05-gate.md`](tasks/w5-05-gate.md) | không có lời gọi mạng ($0); đúc bundle `0.2.1` + 2 lần chạy gate | tiêm 18: **18 đỏ**, không phép nào sống sót | [`runs/gate-0.2.1.html`](runs/gate-0.2.1.html) · [`runs/gate-0.2.1-no-champion.json`](runs/gate-0.2.1-no-champion.json) · [`bundles/rag-bundle-v0.2.1/manifest.json`](../../bundles/rag-bundle-v0.2.1/manifest.json) | ⭐⭐ **INCOMPARABLE là phán quyết thứ ba** (exit 2), không phải một kiểu FAIL; ⭐⭐ `evaluated_with_generator` của hai bundle cũ là **bí danh** `deepseek-chat@2026-09` — trường bảo đảm danh tính lại được gõ tay; ⭐⭐ ngưỡng 3500 ms end-to-end từng so nhầm với p95 **truy hồi** 759 ms, đo lại trên 242 request thật: **4706 ms** |
