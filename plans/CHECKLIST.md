...
| W2 · Retrieval upgrade | 10 | 8 | 0 | 0 | 0 | 2 | `G2` ⬜ |
...
| **Tổng backlog gốc** | **72** | **22** | **0** | **2** | **2** | **47** | 0/6 |
| §9 Task thêm mới (`NEW-xx`) | 6 | 6 | 0 | 0 | 0 | 0 | — |
| **Tổng cộng** | **78** | **28** | **0** | **2** | **2** | **47** | 0/6 |
...
>
> ### Việc tiếp theo: `W2-08` — ablation ≥ 12 tổ hợp, có `p`/CI từng dòng
>
> Cả hai điều kiện đã xong: `W2-07` cho **14 ô** dữ liệu (`runs/e1-*` + MLflow), và
> `W2-08-prep` cho `make eval-compare-by` để có `p`/CI **cho từng dòng** kèm chiều
> category/lang mà DoD `W2-09` đòi.
>
> ⚠️ **Ba cái bẫy đã đo được, đừng đâm lại vào:**
> 1. `e1-chunk550-dense` **không so được** recall/nDCG/MAP với 13 ô kia —
>    `n_relevant_mean` 1,9617 vs 1,3828 (`G2`). `compare.py` tự từ chối.
> 2. `table_lookup` (4 câu) **vĩnh viễn** `KHÔNG ĐỦ LỰC` trên metric nhị phân —
>    trần `p` = 0,125. Đừng đọc "trong ngưỡng nhiễu" của nó là "không có hiệu ứng".
> 3. Bảng chia nhóm hiệu chỉnh cho `nhóm × metric`; đọc nó để **loại** giả thuyết
>    thì được, để **chọn người thắng** thì phải nêu tên nhóm **trước** (`--category`).
>
> ⚠️ `W2-09` phải quyết một chuyện `W2-08-prep` cố ý không quyết: **bảng một-cặp vẫn
> không hiệu chỉnh cho 15 metric** (kỳ vọng 0,75 dương giả mỗi bảng). Sửa thì viết
> lại kết luận `W2-01`…`W2-07`; không sửa thì phải nói ra trong report.
>
