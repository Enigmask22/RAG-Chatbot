...
| W2 · Retrieval upgrade | 9 | 6 | 0 | 0 | 0 | 3 | `G2` ⬜ |
...
| **Tổng backlog gốc** | **71** | **20** | **0** | **2** | **2** | **47** | 0/6 |
| §9 Task thêm mới (`NEW-xx`) | 6 | 6 | 0 | 0 | 0 | 0 | — |
| **Tổng cộng** | **77** | **26** | **0** | **2** | **2** | **47** | 0/6 |
...
>
> ### Việc tiếp theo: `W2-07` — experiment runner + MLflow
>
> `W2-08` (ablation ≥ 12 tổ hợp) là thứ cần nó, và `W2-08` giờ có bốn chiều thật:
> `chunk_size` × `embedding` × nhánh truy hồi × `rerank_candidates`. ⚠️ **`doc_type`
> không phải chiều dùng được**: `W2-06` đo được nó khớp 15.814/15.814 point — toàn corpus
> là một loại, nên hai dòng ablation theo nó sẽ giống nhau. `lang` thì thật (59,4%/40,6%).
>
> ⚠️ Trước `W2-08` nên làm `--category`/`--lang` cho `compare.py`: DoD `W2-09` đòi "nhận
> xét về category nào cải thiện nhiều nhất", và thiếu chiều đó đã để một mức tụt có ý nghĩa
> của `W2-04` đi qua không ai thấy (xem đầu §4).
>
