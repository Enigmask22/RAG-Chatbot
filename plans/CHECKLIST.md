...
| W2 · Retrieval upgrade | 10 | 9 | 0 | 0 | 0 | 1 | `G2` ⬜ |
...
>
> ### Việc tiếp theo: `W2-09` — report `exp-001-retrieval.md`
>
> Dữ liệu đã đủ hẳn: `make ablation` cho bảng 14 dòng có `p`/CI từng dòng
> (`compare/ablation-exp-001-ndcg.md`), `make eval-compare-by` cho chiều
> category/lang mà DoD `W2-09` đòi ("ít nhất 2 nhận xét về category nào cải thiện
> nhiều nhất").
>
> ⚠️ **Ba quyết định `W2-08` và `W2-08-prep` cố ý ĐỂ LẠI cho `W2-09`:**
> 1. **Bảng một-cặp vẫn không hiệu chỉnh cho 15 metric** (kỳ vọng 0,75 dương giả
>    mỗi bảng). Sửa thì viết lại kết luận `W2-01`…`W2-07`; không sửa thì phải nói ra.
> 2. **`MIN_TAIL_RESAMPLES = 30` hiện chỉ để GIẢI THÍCH cờ, chưa để tự nâng `B`.**
>    Đọc được một khoảng ở α = 0,00128 cần ~47.000 mẫu lại (~4,7× chi phí). Đo rồi
>    hãy quyết.
> 3. **Đỉnh bảng `rc100` vs `rc50` là đánh đổi vùng phủ vs 1,91× độ trễ**, và cả ba
>    metric chính đều là metric *xếp hạng* nên chúng loại trừ đúng chiều `rc100`
>    thắng. Chốt điểm vận hành cần biết bộ sinh có dùng được thêm bằng chứng hay
>    không — nếu `W2-09` không chốt được thì nó thuộc `W4`.
>
> ⚠️ **Hai cái bẫy vẫn còn nguyên, đừng đâm lại vào:**
> 1. `table_lookup` (4 câu) **vĩnh viễn** `KHÔNG ĐỦ LỰC` trên metric nhị phân —
>    trần `p` = 0,125. Đừng đọc "trong ngưỡng nhiễu" của nó là "không có hiệu ứng".
