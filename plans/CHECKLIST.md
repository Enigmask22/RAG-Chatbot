...
> Cập nhật lần cuối: 2026-08-22 · Trạng thái tổng: **tuần 1: 13/13 task xong · `golden_v1` 242 câu (review bằng MODEL, không phải người) · baseline recall@5 = 0,1746 · gate `G1` 4/4 ⚠️ với một điều kiện · `W2` 10/10 ✅ · `W3` 1/9 · 1277 test (1276 passed, 1 skipped có chủ đích)**
...
>
> ### Việc tiếp theo: `W3-02` (OCR fallback) hoặc `TD-22` (ghim văn bản đã parse)
>
> `W3` **1/9** sau `W3-01`. Hai hướng, và thứ tự phụ thuộc vào việc có định
> nối loader vào đường index sớm hay không:
>
> * **`TD-22` trước** nếu muốn `W3-07` (incremental re-index) chạy được — nối
>   loader vào `corpus_loader.py` mà chưa ghim `text_sha256` là mở đúng cái lỗ
>   `W3-01` §3 đo được (**0/280 span sống sót**).
> * **`W3-02` trước** nếu muốn đi tiếp theo thứ tự plan. Nó thừa hưởng nguyên
>   quyết định `do_ocr=False` của `W3-01` cùng con số **70,56 s vs 0,12–0,77 s**,
>   nên phần "vì sao cần phát hiện scan" đã có sẵn bằng chứng.
>
> ⚠️ **`W3-03` phải đọc `reports/tasks/w3-01-docling-loader.md` §4 trước khi bắt
> đầu**: `item.level` của docling **không dùng thẳng được** (cùng một heading là
> cấp 3 khi tới từ DOCX và cấp 2 khi tới từ HTML), và PPTX/XLSX không mang phân
> cấp heading nào.
>
...
- [x] `W3-01` **Docling loader** (PDF, DOCX, PPTX, XLSX, HTML, MD) + bảng → Markdown + heading hierarchy — **2026-08-22**
  ...
  · 💡 **OCR tắt mặc định, có số đo**: `do_ocr=True` (mặc định của docling) tốn **70,56 s** cho fixture 1 trang, `do_ocr=False` tốn **0,12–0,77 s** — hai bậc độ lớn cho một PDF born-digital vốn đã có text layer. Bật OCR theo phát hiện scan là DoD của `W3-02`
