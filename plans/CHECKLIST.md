...
| `TD-12` ⭐⭐ | **`chunk_id` thuần vị trí (`doc_id::00094`), không lưu offset ký tự.** Đổi `chunk_size` là mọi `relevant_chunk_ids` của golden set trỏ vào **văn bản khác** — không phải con trỏ chết báo lỗi, mà con trỏ SAI vẫn được chấm điểm. Đo thật trên 1 tài liệu: 1000→600 ký tự cho 46 chunk_id tồn tại ở cả hai cấu hình, **0 chunk_id có nội dung giống nhau, 0 chunk_id biến mất** → `fetch_chunks` báo sạch, `gold_chunk_missing` = 0, không gì cảnh báo | Chưa lộ ra vì index chưa build lại lần nào từ khi sinh nháp | **Phải quyết trước khi bỏ 6–8 giờ review tay.** `TD-11` nói việc đầu tiên của `W2` là hạ `chunk_size` — tức golden set vừa đóng băng sẽ hỏng ngay ở bước sau. Hai đường: (a) review bằng chunk_id rồi ánh xạ lại ở `W2` qua `supporting_quotes` (250/266 câu có trích dẫn đã xác minh); (b) thêm `start_char`/`end_char` vào `Chunk`, build lại index, gán nhãn theo **span ký tự** → golden set độc lập với mọi cấu hình chunking về sau |
